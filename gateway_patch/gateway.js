(function () {
  ...
  const DS_PIN_KEY = 'gatewayPinnedDatasets';
  ...
  function getDsPinned() {
    try { return JSON.parse(localStorage.getItem(DS_PIN_KEY)) || []; } catch { return []; }
  }
  function setDsPinned(v) { localStorage.setItem(DS_PIN_KEY, JSON.stringify(v)); }

  function isPinned(key) { return getPinned().includes(key); }
  function isFolded(key) { return getFolded().includes(key); }
  function isDsPinned(descriptor) { return getDsPinned().includes(descriptor); }
  ...
  function render() {
    ...
    filtered.sort(cmp);
    filtered.sort((a, b) => {
      const aPinned = isDsPinned(a.descriptor);
      const bPinned = isDsPinned(b.descriptor);
      ...
      html += `<div class="folder-section">
        ...
        <div class="folder-content" id="${safeId}" style="display: ${isFoldedFolder ? 'none' : 'grid'}">
          <div class="dataset-grid">
            ${datasets.map(ds => {
              return `<div class="dataset-card ${isDsPinned(ds.descriptor) ? 'pinned' : ''}" data-descriptor="${ds.descriptor}">
                <div class="card-title">
                  <button class="btn btn-sm btn-link ds-pin-btn p-0 me-1 ${isDsPinned(ds.descriptor) ? 'text-warning' : 'text-secondary'}" data-descriptor="${ds.descriptor}" title="${isDsPinned(ds.descriptor) ? 'Unpin' : 'Pin'}">
                    <i class="bi ${isDsPinned(ds.descriptor) ? 'bi-pin-fill' : 'bi-pin'}"></i>
                  </button>
    ...
    document.querySelectorAll('.ds-pin-btn').forEach(btn => {
      btn.addEventListener('click', function (e) {
        e.stopPropagation();
        const descriptor = this.dataset.descriptor;
        const pinnedSet = getDsPinned();
        const idx = pinnedSet.indexOf(descriptor);
        if (idx > -1) { pinnedSet.splice(idx, 1); }
        else { pinnedSet.push(descriptor); }
        setDsPinned(pinnedSet);
        render();
      });
    });
